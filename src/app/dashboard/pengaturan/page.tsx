...
import { getUsers, addUser, deleteUser, updateUserPermissions } from "@/app/actions/user";
...
export default function SettingsPage() {
  ...
  const [customUserPerms, setCustomUserPerms] = useState<string[] | null>(null);
  ...
  const handleSaveUserPermissions = async () => {
    ...
    const res = await updateUserPermissions(selectedUserForPerms.id, customUserPerms);
    if (res.success) {
      alert("Hak akses pengguna berhasil diperbarui.");
      setShowUserPermsModal(false);
      await loadData();
    } else {
      alert("Gagal memperbarui hak akses: " + res.error);
    }
    setUserPermsSaving(false);
  };

  const toggleCustomUserPermission = (menuName: string) => {
    setCustomUserPerms(prev => {
      const current = prev || [];
      return current.includes(menuName) 
  ...
  const availableMenus = [
    ...
    "Jemaat", "Ibadah", "Pelayanan", "Sakramen", "Visitasi", "Komsel", 
  ...
  return (
    <div className="space-y-10 animate-fade-in-up pb-12 font-sans max-w-4xl">
                      ...
                      <div className="flex justify-center items-center gap-2">
                        <button 
                          onClick={() => {
                            ...
                            setCustomUserPerms(user.customPermissions || null);
                            setShowUserPermsModal(true);
                          }}
                          className="px-3 py-1.5 text-xs font-bold bg-zinc-100 hover:bg-zinc-200 text-zinc-700 rounded-lg transition-colors border border-zinc-200"
                        >
                          Hak Akses
                        </button>
                        <button 
      ...
      {/* ── MODAL HAK AKSES USER ─────────────────────────────────────────── */}
      {showUserPermsModal && selectedUserForPerms && (
        <Portal>
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-zinc-900/60 backdrop-blur-sm animate-fade-in">
            <div className="bg-white rounded-3xl shadow-2xl w-full max-w-2xl overflow-hidden animate-fade-in-up border border-zinc-200/60 flex flex-col max-h-[90vh]">
              <div className="p-6 md:p-8 overflow-y-auto">
                <div className="flex justify-between items-center mb-6">
                  <div>
                    <h2 className="text-xl font-extrabold text-zinc-900">Hak Akses Kustom: {selectedUserForPerms.name}</h2>
                    <p className="text-xs text-zinc-500 font-medium mt-1">Role: {selectedUserForPerms.role}</p>
                  </div>
                  <button 
                    onClick={() => setShowUserPermsModal(false)}
                    className="w-8 h-8 flex items-center justify-center rounded-full bg-zinc-100 text-zinc-500 hover:bg-zinc-200 transition-colors shrink-0"
                  >
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
                  </button>
                </div>
                ...
                <div className="space-y-6">
                  <div className="flex items-center gap-3 p-4 bg-zinc-50 border border-zinc-200 rounded-xl cursor-pointer hover:border-zinc-300 transition-colors" onClick={() => setCustomUserPerms(customUserPerms === null ? (rolePermissions[selectedUserForPerms.role] || []) : null)}>
                    <div className="relative flex items-center justify-center w-5 h-5">
                      <input
                        type="checkbox"
                        checked={customUserPerms !== null}
                        readOnly
                        className="peer appearance-none w-5 h-5 border-2 border-zinc-300 rounded focus:ring-2 focus:ring-zinc-900 focus:outline-none checked:border-zinc-900 checked:bg-zinc-900 transition-all cursor-pointer"
                      />
                      <svg className="absolute w-3.5 h-3.5 text-white opacity-0 peer-checked:opacity-100 pointer-events-none transition-opacity" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}>
                    ...
                    <div>
                      <div className="text-sm font-bold text-zinc-900">Gunakan Hak Akses Kustom</div>
                      <div className="text-xs text-zinc-500 mt-0.5">Jika diceklis, user ini akan menggunakan izin di bawah alih-alih izin default dari Role {selectedUserForPerms.role}.</div>
                    </div>
                  </div>

                  {customUserPerms !== null && (
                    <div className="border border-zinc-200 rounded-xl p-5">
                      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                        {availableMenus.map(menu => {
                          const isChecked = customUserPerms.includes(menu);
                          return (
                            <label key={menu} className="flex items-center gap-3 p-3 bg-white border border-zinc-200 rounded-xl cursor-pointer hover:border-zinc-400 transition-colors">
                              <div className="relative flex items-center justify-center w-5 h-5">
                                <input
                                  type="checkbox"
                                  checked={isChecked}
                                  onChange={() => toggleCustomUserPermission(menu)}
                                  className="peer appearance-none w-5 h-5 border-2 border-zinc-300 rounded focus:ring-2 focus:ring-zinc-900 focus:outline-none checked:border-zinc-900 checked:bg-zinc-900 transition-all cursor-pointer"
                                />
                                <svg className="absolute w-3.5 h-3.5 text-white opacity-0 peer-checked:opacity-100 pointer-events-none transition-opacity" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}>
                                  <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
                  ...
                  <div className="flex justify-end gap-3 pt-4 border-t border-zinc-100">
                    <button type="button" onClick={() => setShowUserPermsModal(false)} className="px-5 py-2.5 text-zinc-500 font-bold hover:bg-zinc-100 rounded-xl transition-colors text-sm">Batal</button>
                    <button onClick={handleSaveUserPermissions} disabled={userPermsSaving} className="px-6 py-2.5 bg-zinc-900 hover:bg-zinc-800 text-white font-bold rounded-xl transition-all shadow-md flex items-center gap-2 text-sm">
                      {userPermsSaving ? 'Menyimpan...' : 'Simpan Hak Akses'}
                    </button>
                  </div>
                </div>
              </div>
